...
import com.be.recheckbe.global.config.S3Config;
...
public class S3ServiceImpl implements S3Service {
  ...
  public String uploadFile(PathName pathName, MultipartFile file) {
    ...
    String fileName =
        pathName.getPath() + "/" + UUID.randomUUID() + extractExtension(file.getOriginalFilename());
  ...
  public void deleteFile(String fileUrl) {
    ...
    DeleteObjectRequest deleteObjectRequest =
        DeleteObjectRequest.builder().bucket(s3Config.getBucket()).key(fileName).build();
  ...
  private String getFileUrl(String fileName) {
    return String.format(
        "https://%s.s3.ap-northeast-2.amazonaws.com/%s", s3Config.getBucket(), fileName);
  }
}
