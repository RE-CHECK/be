...
public class CollegesDataInitializer implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
        ...
        Map.ofEntries(
            Map.entry(
                ...
                List.of(
                    ...
                    "교통시스템공학과",
                    "건축학과")),
            Map.entry("첨단ICT융합대학", List.of("전자공학과", "지능형반도체공학과", "미래모빌리티공학과")),
            Map.entry("소프트웨어융합대학", List.of("소프트웨어학과", "사이버보안학과", "디지털미디어학과", "국방디지털융합학과")),
            Map.entry("자연과학대학", List.of("수학과", "프런티어과학학부")),
            Map.entry("경영대학", List.of("경영학과", "경영인텔리전스학과", "금융공학과")),
            Map.entry("인문대학", List.of("국어국문학과", "영어영문학과", "불어불문학과", "사학과", "문화콘텐츠학과")),
            Map.entry("사회과학대학", List.of("행정학과", "심리학과", "스포츠레저학과", "경제정치사회융합학부")),
    ...
    for (String collegeName : collegeOrder) {
      College college = collegeRepository.save(College.builder().name(collegeName).build());

      for (String deptName : data.get(collegeName)) {
        departmentRepository.save(Department.builder().name(deptName).college(college).build());
      }
    }
  }
}
