...
> `reason`은 선택 항목입니다. 관리자 메모 용도로 내부에만 저장됩니다.
...
**PATCH** `/api/admin/blacklist/{blacklistId}/unban` `🔒 관리자 계정 인증 필요`

블랙리스트를 해제하여 해당 전화번호로 재가입을 허용합니다. 차단 이력은 DB에 보존됩니다.

**Path Parameter**

| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `blacklistId` | Long | 블랙리스트 항목 ID (`blacklist/csv` 다운로드로 확인) |
